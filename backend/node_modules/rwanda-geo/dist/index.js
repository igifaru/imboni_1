...
function getPackageRoot() {
  try {
    ...
    while (currentDir2 !== path.dirname(currentDir2)) {
      if (fs.existsSync(path.join(currentDir2, "package.json"))) {
        if (fs.existsSync(path.join(currentDir2, "dist", "data", "provinces.json.gz"))) {
          return currentDir2;
        }
      }
      currentDir2 = path.dirname(currentDir2);
    }
...
function lazyLoadGzippedJson(filename) {
  ...
  const paths = [
    path.join(packageRoot, "dist/data", `${filename}.json.gz`),
    // Production (published package)
    path.join(packageRoot, "src/data", `${filename}.json.gz`),
    // Development
    path.join(packageRoot, "dist/data", `${filename}.json`),
    // Production fallback
    path.join(packageRoot, "src/data", `${filename}.json`)
  ...
  for (const path of paths) {
    try {
      if (fs.existsSync(path)) {
        if (path.endsWith(".gz")) {
          const gzipped = fs.readFileSync(path);
...
function getModuleDir() {
  if (typeof __dirname !== "undefined") {
    ...
    return path.dirname(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.js', document.baseURI).href))));
  }
}
function getDataPaths(filename) {
  ...
  return [
    ...
    path.join(moduleDir, "data", `${filename}.json.gz`),
    path.join(moduleDir, "data", `${filename}.json`),
    // Development: src/data (when running from src/index.ts)
    path.join(moduleDir, "../src/data", `${filename}.json.gz`),
    path.join(moduleDir, "../src/data", `${filename}.json`)
  ];
}
...
exports.AdminLevel = AdminLevel;
exports.clearDataCache = clearDataCache;
exports.default = index_default;
exports.fuzzySearchByName = fuzzySearchByName;
...
exports.getFullHierarchy = getFullHierarchy;
...
exports.loadGzippedJson = loadGzippedJson;
exports.loadGzippedJsonSafe = loadGzippedJsonSafe;
exports.preloadData = preloadData;
exports.searchByName = searchByName;
exports.searchByPartialCode = searchByPartialCode;
...
exports.validateUnitProperties = validateUnitProperties;
